...
export default SearchString = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const loadData = async (trantype) => {
            ...
            if (keyword !== null && keyword !== '') {
                url = `${url}?keyword=${keyword}&&type=${trantype}`;
            }
            ...
            {weight === null ? <ActivityIndicator /> : <>
                <View style={{ flexDirection: 'row', justifyContent: 'space-evenly', marginTop: 10, marginBottom: 2 }}>
                    <View style={styles.TitleTotal}>
                        ...
                        <Text style={{ fontSize: 28, fontWeight: '700', textAlign: 'center', color: 'red' }}>{count}</Text>
                    </View>

                    <View style={styles.TolalWeight}>
                        <Text style={{ fontSize: 17, fontWeight: '700', textAlign: 'center' }}>Tổng trọng lượng hàng</Text>
                        <Text style={{ fontSize: 28, fontWeight: '700', textAlign: 'center', color: 'red' }}>{formatCurrency(calculateTotalWeight(weight))}</Text>
                    </View>
                </View>
                ...
                {weight.map(w => (
                    <View key={w.id}>
                        <TouchableOpacity style={styles.ItemWeight} onPress={() => goToWeightDetail(w.id)}>
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                                <View style={{ flexDirection: 'row' }}>
                                    <Text style={{ fontSize: 17 }}>Mã phiếu: </Text>
                                    <Text style={{ fontSize: 17, marginLeft: 5, fontWeight: '800', color: 'red' }}>{w.Ticketnum}</Text>
                                </View>
                            </View>

                            <View style={{ flexDirection: 'row', marginTop: 4 }}>
                                <Text style={{ fontSize: 17 }}>Ngày tạo phiếu: </Text>
                                <Text style={{ fontSize: 17, marginLeft: 5, fontWeight: '700' }}>{moment(w.date_time).utcOffset(7).format('DD/MM/YYYY HH:mm:ss')}</Text>
                            </View>

                            <View style={{ flexDirection: 'row', marginTop: 4 }}>
                                <Text style={{ fontSize: 17 }}>Trọng lượng thực: </Text>
                                <Text style={{ fontSize: 17, marginLeft: 5, fontWeight: '700' }}>{formatCurrency(w.Netweight)}</Text>
                            </View>
                        </TouchableOpacity>
