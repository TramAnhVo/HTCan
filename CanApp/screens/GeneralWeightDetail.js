                ...
                {data === null ? <ActivityIndicator /> : <>
                    <View style={{ flexDirection: 'row', marginTop: 10, marginBottom: 0, justifyContent: 'space-between' }}>
                        <View style={styles.titleDate}>
                            ...
                            <Text style={{ color: 'white', fontWeight: '700', marginLeft: 3, fontSize: 13 }}>
                                {moment(data.created_day).utcOffset(7).format('DD-MM-YYYY')}</Text>
                        </View>

                        <TouchableOpacity style={styles.buttonDetail} onPress={() => goToDetail()}>
                            <Text style={{ color: 'white', fontSize: 13, fontWeight: '700' }}>Xem chi tiết</Text>
                        </TouchableOpacity>
                    </View>

                    <View style={{ marginBottom: 10 }}>
                        <ChartPie countIn={data.count_in} countOut={data.count_out} />
                        <Text style={{ textAlign: 'center', fontSize: 13 }}>Biểu đồ về số lượng phiếu trong ngày</Text>
                    </View>

                    <View style={[styles.item, style = { marginTop: 10 }]}>
                        <View style={{
                            width: '43%', backgroundColor: 'lightblue', padding: 10, height: 65, borderRadius: 5,
                            ...
                            <Text style={{ textAlign: 'center', fontSize: 13, fontWeight: '800' }}>{data.CountWeight}</Text>
                        </View>

                        <View style={{
                            width: '53%', backgroundColor: 'lightblue', padding: 10, height: 65, borderRadius: 5,
                            shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.8, shadowRadius: 5, elevation: 4
                        }}>
                            <Text style={{ textAlign: 'center', fontSize: 13, fontWeight: '600' }}>Tổng trọng lượng</Text>
                            <Text style={{ textAlign: 'center', fontSize: 13, fontWeight: '800' }}>{formatCurrency(data.TotalWeight || 0)}</Text>
                        </View>
                    </View>

                    <View style={styles.item}>
                        <View style={{
                            width: '43%', backgroundColor: 'lightgreen', padding: 10, height: 65, borderRadius: 5,
                            shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.8, shadowRadius: 5, elevation: 4
                        }}>
                            <Text style={{ textAlign: 'center', fontSize: 13, fontWeight: '600' }}>Tổng phiếu nhập</Text>
                            <Text style={{ textAlign: 'center', fontSize: 13, fontWeight: '800' }}>{data.count_in}</Text>
                        </View>

                        <View style={{
                            width: '53%', backgroundColor: 'lightgreen', padding: 10, height: 65, borderRadius: 5,
                            shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.8, shadowRadius: 5, elevation: 4
                        }}>
                            <Text style={{ textAlign: 'center', fontSize: 13, fontWeight: '600' }}>Tổng trọng lượng nhập</Text>
                            <Text style={{ textAlign: 'center', fontSize: 13, fontWeight: '800' }}>{formatCurrency(data.total_in || 0)}</Text>
                        </View>
                    </View>

                    <View style={styles.item}>
                        <View style={{
                            width: '43%', backgroundColor: 'pink', padding: 10, height: 65, borderRadius: 5,
                            shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.8, shadowRadius: 5, elevation: 4
                        }}>
                            <Text style={{ textAlign: 'center', fontSize: 13, fontWeight: '600' }}>Tổng phiếu xuất</Text>
                            <Text style={{ textAlign: 'center', fontSize: 13, fontWeight: '800' }}>{data.count_out}</Text>
                        </View>

                        <View style={{
                            width: '53%', backgroundColor: 'pink', padding: 10, height: 65, borderRadius: 5,
                            shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.8, shadowRadius: 5, elevation: 4
                        }}>
                            <Text style={{ textAlign: 'center', fontSize: 13, fontWeight: '600' }}>Tổng trọng lượng xuất</Text>
                            <Text style={{ textAlign: 'center', fontSize: 13, fontWeight: '800' }}>{formatCurrency(data.total_out || 0)}</Text>
                        </View>
                    </View>
                </>}
            </ScrollView>
            ...
            <Tab.Navigator>
                <Tab.Screen name="GeneralChart" component={GeneralChart}
                    options={{
                        ...
                        tabBarLabel: ({ focused }) => (
                            <Text style={{ fontSize: 11 }}>THỐNG KÊ CHUNG</Text> // Thay đổi kích thước chữ tại đây
                        ),
                    }} />
                <Tab.Screen name="SearchCustomer" component={SearchCustomer}
                    options={{
                        ...
                        tabBarLabel: ({ focused }) => (
                            <Text style={{ fontSize: 11 }}>THỐNG KÊ KHÁCH HÀNG</Text> // Thay đổi kích thước chữ tại đây
                        ),
                    }} />
            </Tab.Navigator>
        </View>
