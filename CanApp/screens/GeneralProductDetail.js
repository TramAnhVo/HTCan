...
export default GeneralProductDetail = ({ navigation, route }) => {
    const [weight, setWeight] = useState(null)
    const [groups, setGroups] = useState([]);

    const [loading, setLoading] = useState(true);
    const Tab = createMaterialTopTabNavigator();
    const { scaleId, day, month, year, ProdName, ProdCode } = route.params;
    ...
    const loadWeightDetail = async () => {
        try {
            let res = await Api.get(endpoints['GeneralProduct'](year, month, day, scaleId, ProdCode));
            const data = res.data
            setWeight(res.data);
    ...
    useEffect(() => {
        loadWeightDetail();
    }, [year, month, day, scaleId, ProdCode])
        ...
        data.forEach((item) => {
            if (!groups[item.CustCode]) {
                groups[item.CustCode] = {
                    code: item.CustCode, nameProd: item.CustName,
                    count: 0, total: 0,
            ...
            if (item.Trantype === 'Xuất hàng') {
                ...
                groups[item.CustCode].totalXuat += item.Netweight;
            } else if (item.Trantype === 'Nhập hàng') {
                groups[item.CustCode].phieuNhap++;
                groups[item.CustCode].totalNhap += item.Netweight;
            }
        });
    ...
    const exportToXLSX = async () => {
        try {
            ...
            const filename = FileSystem.documentDirectory + "Data_" + day + "_" + month + "_" + year + "_" + ProdName + ".xlsx";
        ...
        return (
            <ScrollView>
                {weight === null ? <ActivityIndicator /> : <>
                    ...
                    {/* thanh tim kiem */}
                    <View style={{ alignItems: 'center', flexDirection: 'row', height: 55, justifyContent: 'center' }}>
                        <TextInput
                        ...
                        {/* reload */}
                        <TouchableOpacity style={[styles.ItemSreach, style = { width: '6%' }]} onPress={() => TrantypeWeight('All')}>
                        ...
                        <View style={{ width: '45%' }}>
                            <Dropdown
                                ...
                                onChange={item => {
                                    ...
                                    setWeight(weightItem.filter(data => data.CustCode === item.CustCode))
                                    setDataCustomer(weightItem.filter(data => data.CustCode === item.CustCode))
    ...
    const GeneralProduct = () => {
                ...
                <View style={{ marginHorizontal: 15, marginVertical: 2, padding: 8 }}>
                    <View>
                        ...
                        <View style={{ flexDirection: 'row', marginLeft: 14 }}>
                            <Text style={{ fontSize: 14, marginVertical: 2 }}>Tên khách hàng: </Text>
        ...
        return (
            <View style={{ flex: 1, backgroundColor: 'white' }}>
                ...
                <View style={{ backgroundColor: 'lightblue', padding: 10, backgroundColor: 'white' }}>
                    <View style={{ flexDirection: 'row' }}>
                        <Text>Ngày: </Text>
                    ...
                    <View style={{ flexDirection: 'row', marginTop: 3 }}>
                        <Text>Mã hàng hóa: </Text>
                        <Text style={{ fontSize: 14, fontWeight: '700', color: 'red' }}>{ProdCode}</Text>
                    </View>

                    <View style={{ flexDirection: 'row', marginTop: 3 }}>
                        <Text>Tên hàng hóa: </Text>
                        <Text style={{ fontSize: 14, fontWeight: '700' }}>{ProdName}</Text>
                    </View>
                </View>
    ...
    return (
        <View style={{ flex: 1 }}>
            {loading ? (
                ...
                <Tab.Navigator>
                    <Tab.Screen name="GeneralProduct" component={GeneralProduct} options={{ title: 'Thống kê chung' }} />
                    <Tab.Screen name="WeightProductDetail" component={WeightProductDetail} options={{ title: 'Phiếu cân chi tiết' }} />
                </Tab.Navigator>
            )}
        </View>
    )
