...
export default SearchTime = ({ route, navigation }) => {
    ...
    useEffect(() => {
        const loadData = async (type) => {
            let res = await Api.get(endpoints["SreachForTime"](time, type));
            setWeight(res.data)
        }
    ...
    return (
        <ScrollView style={{ flex: 1}}>
            {weight === null ? <ActivityIndicator /> : <>
                ...
                <View style={[style = { flexDirection: 'row', justifyContent: 'space-around' }, styles.ItemDate]}>
                    <View style={{ flexDirection: 'row' }}>
                        <Text style={{ fontSize: 16, marginRight: 5 }}>Từ ngày:</Text>
                        <Text style={{ fontSize: 16, fontWeight: '500' }}>{moment(weight.NgayTimKiem.start, 'YYYY-MM-DD').format('DD-MM-YYYY')}</Text>
                    </View>

                    <View style={{ flexDirection: 'row' }}>
                        <Text style={{ fontSize: 16, marginRight: 5 }}>Đến ngày:</Text>
                        <Text style={{ fontSize: 16, fontWeight: '500' }}>{moment(weight.NgayTimKiem.end, 'YYYY-MM-DD').format('DD-MM-YYYY')}</Text>
                    </View>
                </View>
