...
export default SearchCustomer = ({ navigation, route }) => {
    ...
    const loadWeightDetail = async () => {
        try {
            ...
            const data = groupByCustomer(res.data, valueItem)       
            setWeight(data)
        } catch (ex) {
            console.error(ex);
        } finally {
    ...
    const groupByCustomer = (data, value) => {
        const grouped = {
            CustCode: value,
            CustName: '',
            totalWeight: 0,
        ...
        data.forEach(item => {
            if (item.CustCode === value) {
                grouped.CustName = item.CustName; // Lưu tên khách hàng
                ...
                grouped.groupedByDate[day].categoryProduct.add(item.ProdCode);
            }
        });
    ...
    const goToDetail = (date) => {
        ...
        navigation.navigate('GeneralCustomerDetail', { "scaleId": value, "day": day, "month": month, "year": year, "custCode": weight.CustCode, "custName": weight.CustName });
    }
            ...
            <ScrollView>
                <View style={{ marginHorizontal: 10, height: 80, padding: 6 }}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginVertical: 3 }}>
                        ...
                        <Text style={{ fontSize: 14, fontWeight: '700', color: 'red' }}>{weight.CustCode}</Text>
                    </View>

                    <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                        <Text style={{ fontSize: 14 }}>Tên khách hàng: </Text>
                        <Text style={{ fontSize: 14, fontWeight: '700' }}>{weight.CustName}</Text>
                    </View>
                </View>
                ...
                <View style={[styles.item, style = { marginTop: 10 }]}>
                    <View style={{
                        width: '43%', backgroundColor: 'lightblue', padding: 10, height: 65, borderRadius: 5,paddingHorizontal: 5,
                    ...
                    <View style={{
                        width: '53%', backgroundColor: 'lightblue', padding: 10, height: 65, borderRadius: 5,paddingHorizontal: 5,
                        shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.8, shadowRadius: 5, elevation: 4
                    }}>
                ...
                <View style={styles.item}>
                    <View style={{
                        width: '43%', backgroundColor: 'lightgreen', padding: 10, height: 65, borderRadius: 5,paddingHorizontal: 5,
                        shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.8, shadowRadius: 5, elevation: 4
                    }}>
                    ...
                    <View style={{
                        width: '53%', backgroundColor: 'lightgreen', padding: 10, height: 65, borderRadius: 5,paddingHorizontal: 5,
                        shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.8, shadowRadius: 5, elevation: 4
                    }}>
                ...
                <View style={styles.item}>
                    <View style={{
                        width: '43%', backgroundColor: 'pink', padding: 10, height: 65, borderRadius: 5,paddingHorizontal: 5,
                        shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.8, shadowRadius: 5, elevation: 4
                    }}>
                    ...
                    <View style={{
                        width: '53%', backgroundColor: 'pink', paddingHorizontal: 5, paddingVertical: 10 , height: 65, borderRadius: 5,
                        shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.8, shadowRadius: 5, elevation: 4
                    }}>
                    ...
                    (weight.groupedByDate.map((dateGroup, index) => (
                        <TouchableOpacity style={styles.itemDate} key={dateGroup.code} onPress={() => goToDetail(dateGroup.code)}>
                            ...
                            <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 2 }}>
                                ...
                                <Text>Loại mặt hàng: {dateGroup.categoryProduct}</Text>
                            </View>
